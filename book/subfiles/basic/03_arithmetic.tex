...
% - restructure (introduce numbers earlier, and move rounding and scientific notation)
% - inequalities http://www.mathsisfun.com/algebra/inequality.html
% - absolute value
% - evaluate: 
% -: -1 - (-z) - 5 - (-3) where z=-2 (answer = - 5)
% -: x- (-y) where x=-2 and y=5 (answer 3)
% -: 3 - (-6) + (-h) + (-4) (answer 12)
...
We also see that when a decimal is multiplied by 10, every figure moves one place to the left ($1.25 * 10 = 12.5$) and when you divide by 10, every figure moves one place to the right ($1.25 * 1/10 = 0.125$).
